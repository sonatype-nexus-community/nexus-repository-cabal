...
-->
# Nexus Repository Cabal Format

[![Maven Central](https://img.shields.io/maven-central/v/org.sonatype.nexus.plugins/nexus-repository-cabal.svg?label=Maven%20Central)](https://search.maven.org/search?q=g:%22org.sonatype.nexus.plugins%22%20AND%20a:%22nexus-repository-cabal%22) [![CircleCI](https://circleci.com/gh/sonatype-nexus-community/nexus-repository-cabal.svg?style=shield)](https://circleci.com/gh/sonatype-nexus-community/nexus-repository-cabal) [![Join the chat at https://gitter.im/sonatype/nexus-developers](https://badges.gitter.im/sonatype/nexus-developers.svg)](https://gitter.im/sonatype/nexus-developers?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge) [![DepShield Badge](https://depshield.sonatype.org/badges/sonatype-nexus-community/nexus-repository-cabal/depshield.svg)](https://depshield.github.io)
...
If everything checks out, the bundle for cabal should be available in the `target` folder

#### Build with Docker

    docker build -t nexus-repository-cabal .

#### Run as a Docker container

    docker run -d -p 8081:8081 --name nexus-repository-cabal nexus-repository-cabal 
...
To read the generated admin password for your first login to the web UI, you can use the commands below against the running docker container:
    ...
    docker exec -it nexus-repository-cabal /bin/bash
...
All released versions can be found [here](https://github.com/sonatype-nexus-community/nexus-repository-cabal/releases).
...
You should see `cabal (proxy)` in the available Repository Recipes to use, if all has gone according to plan :)
...
* Run Nexus Repo console:
  ...
  $ cd <nexus_dir>/bin
  $ ./nexus run
  > bundle:install file:///tmp/nexus-repository-cabal-0.0.1.jar
  > bundle:list
  ```
  (look for org.sonatype.nexus.plugins:nexus-repository-cabal ID, should be the last one)
  ```
  > bundle:start <org.sonatype.nexus.plugins:nexus-repository-cabal ID>
  ```

### (more) Permanent Install

For more permanent installs of the nexus-repository-cabal plugin, follow these instructions:

* Copy the bundle (nexus-repository-cabal-0.0.1.jar) into <nexus_dir>/deploy
...
If you are trying to use the cabal plugin permanently, it likely makes more sense to do the following:

* Copy the bundle into `<nexus_dir>/system/org/sonatype/nexus/plugins/nexus-repository-cabal/0.0.1/nexus-repository-cabal-0.0.1.jar`
* Make the following additions marked with + to `<nexus_dir>/system/org/sonatype/nexus/assemblies/nexus-core-feature/3.x.y/nexus-core-feature-3.x.y-features.xml`

   ```
         <feature prerequisite="false" dependency="false">wrap</feature>
   +     <feature prerequisite="false" dependency="false">nexus-repository-cabal</feature>
   ```
   ...
   And
   ```
   + <feature name="nexus-repository-cabal" description="org.sonatype.nexus.plugins:nexus-repository-cabal" version="0.0.1">
   +     <details>org.sonatype.nexus.plugins:nexus-repository-cabal</details>
   +     <bundle>mvn:org.sonatype.nexus.plugins/nexus-repository-cabal/0.0.1</bundle>
   + </feature>
    </features>
   ```
   as the last feature.
...
The project has a “format” module, and an “IT” module. 
   ...
   In this project, the sub module: [nexus-repository-cabal](nexus-repository-cabal) is the "format" module.
   The sub module: [nexus-repository-cabal-it](nexus-repository-cabal-it) is the "it" module.
...
#### Debugging ITs
  ...
  When running ITs, the Nexus Repository Manager will write log output to the following file:
      ...
      nexus-repository-cabal/nexus-repository-cabal-it/target/it-data/1/nexus3/log/nexus.log
