...
public class TgzParser
{
  ...
  @Nullable
  public InputStream getCabalFileFromTempBlob(final InputStream is) throws IOException {
    try (GzipCompressorInputStream gzis = new GzipCompressorInputStream(is)) {
      try (TarArchiveInputStream tais = new TarArchiveInputStream(gzis)) {
        ...
        while(currentEntry != null) {
          if(currentEntry.getName().contains(CABAL_EXTENSION)) {
  ...
  @Nullable
  public InputStream getCabalFileFromTempBlob(final TempBlob tempBlob) throws IOException {
    return getCabalFileFromTempBlob(tempBlob.get());
  }
}
